...
def _get_export_file_path(ds, file_path, target_root, filetype, anonymous, patient_dict):
    ...
    # Lấy đường dẫn cơ bản và tên file từ file_path
    base_path = Path(file_path).parent
    original_filename = Path(file_path).stem

    # Tạo đường dẫn xuất mới với phần mở rộng đã thay đổi
    export_file_path = base_path / f"{original_filename}.{filetype}"
